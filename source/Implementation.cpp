...
public:
	...
	bool	UnpopAndSwap	(Task* pTask);
	bool	PushFast		(Task* pTask);
	bool	PushExecute		(Task** pTaskArray, u32 count);
	bool	PushExecute		(Task* pTask);
...
public:
	...
	Task*	PopExecute		() {
		...
		if (m_taskExecuteStart == m_taskExecuteEnd) {
			...
			pTask		= m_taskExecuteStart->pTask;
			m_taskExecuteStart++;
...
bool WRK_PushExecute(u8 worker, Task* task) {
	bool res = g_threads[worker].PushExecute(task);
	return res;
}
...
void WRK_TaskYield			(u8 worker, Task* pTask, u8 mode) {
	switch (mode) {
	case 0:
		// Do nothing;
		pTask->TSK_UnsetYield();
	case 1:
		g_threads[worker].UnpopAndSwap(pTask);	break;
		pTask->TSK_SetYield();
		break;
	case 2:
		g_threads[worker].PushExecute(pTask);	break;
		pTask->TSK_SetYield();
		break;
	case 3:
		g_threads[worker].PushFast(pTask);	break;
		pTask->TSK_SetYield();
		break;
	}
}
...
bool WorkThread::StealTasks() {
	...
	while (Offset < endOffset) {
		WorkThread* pThread = &g_threads[ m_group[(Offset >= threadCount) ? (Offset - threadCount) : Offset]];
		Offset++;
...
bool WorkThread::UnpopAndSwap(Task* pTask) {
	bool res = true;
	EC_ASSERT(pTask->m_executeRemainCount==0);
	m_taskArrayLock.Lock();
...
bool WorkThread::PushFast(Task* pTask) {
	bool res = true;
	EC_ASSERT(pTask->m_executeRemainCount==0);
	m_taskArrayLock.Lock();
	...
	if (res) {
		m_taskExecuteStart->pTask	= pTask;
	}
	m_taskArrayLock.Unlock();
	return res;
}

bool WorkThread::PushExecute(Task** pTaskArray, u32 count) {
...
bool WorkThread::PushExecute(Task* pTask) {
	bool res = true;
	EC_ASSERT(pTask->m_executeRemainCount==0);
	m_taskArrayLock.Lock();
	if (m_taskExecuteStart <= m_taskExecuteEnd) {
		m_taskExecuteEnd->pTask = pTask;
		m_taskExecuteEnd++;
...
void WorkThread::Run() {
	...
	if (spyFunc) { spyFunc(spyCtx, worker, NULL); }
	while (1) {
		do
		{
			Task* pTask;
			while (m_bKeepAlive && (pTask = PopExecute())) {
				if (spyFunc) {
					spyFunc(spyCtx, worker, pTask);
				}

				//
				// Splittable.
				//
				if ((pTask->m_attribute & (Task::ATTRB_SPLITTABLE | Task::ATTRB_SPLITTED)) == Task::ATTRB_SPLITTABLE) {
					pTask->m_attribute |= Task::ATTRB_SPLITTED;

					//
					// Split maximum amount of task around all the same group thread.
					//
					u32 splitCount	= g_ThreadCount * 3;
					u32 amount		= pTask->m_streamAmount;
					u32 splitSize	= (amount / splitCount) + 1; // Round up to be sure
					if (splitSize < pTask->m_streamGranularity) {
						splitSize	= pTask->m_streamGranularity;
						splitCount = (amount + (splitSize-1)) / (splitSize);
					}
					
					pTask->m_dependancyLock.Lock();
					pTask->m_splitCount = splitCount;
					pTask->m_splitSize  = splitSize;
					pTask->m_dependancyLock.Unlock();
					...
					while (splitter < splitCount) {
						if (workerIdx >= m_groupCount) {
							...
							pWorkers[m_group[workerIdx++]].PushFast(pTask);
						}
						splitter++;
					}
				}
				...
				(*((CppCall*)pTask).*pTask->m_runFunc)(worker);

				// Execution complete
				// We ensure that multiple thread will not split at the same time.
				// TODO OPTIMIZE, replace later with Atomic Counter
				if (pTask->m_splitCount == 0) {
					if (pTask->m_dependantToCount) { // TODO : for now variable only modified when registering task outside of execution kernel, use Task::ATTRB_PUSHED attribute to garantee.
						DependancyEntry* slot = pTask->m_dependancySlotTo;
						while (slot) {
							slot->m_pToTask->FinishedExecute(pTask, this);
							if (slot->m_snext) {
					...
					// Unset flag anyway, ready for next push 
					// And test for suicide flag.
					if (((pTask->m_attribute &= ~Task::ATTRB_SPLITTED)) & Task::ATTRB_SUICIDE) {
						delete pTask;
					}
				}
			}
		...
		if (spyFunc) {
			if (lastRunCount != m_gotWork) {
				...
				spyFunc(spyCtx, worker, (Task*)0xFFFFFFFF);
			}
		}
...
Task::~Task() {
	...
	while (slot) {
		TSK_UnWaitingForMe(slot->m_pTask);
		if (slot->m_lnext) {
	...
	while (slot) {
		TSK_UnWaitingForTask(slot->m_pTask);
		if (slot->m_lnext) {
			slot = &slot[slot->m_lnext]; // Trick with relative adress
		} else {
			slot = NULL;
...
bool Task::TSK_WaitingForTask(Task* pTask) {
...
void Task::FinishedExecute(Task* pTask, WorkThread* pWorker) {
	m_dependancyLock.Lock();
	if ((--m_executeRemainCount) == 0) {
		// Push as 0 dependancy in the thread of the ended task.
		pWorker->PushExecute(this);
